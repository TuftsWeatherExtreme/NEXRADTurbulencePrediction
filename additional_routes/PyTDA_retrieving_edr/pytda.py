...
Last Updated 03/04/2024
  Note: updated by Team Celestial Blue with a bug fix on 02/13/2025, 
...
def calc_turb_vol(radar, radius=DEFAULT_RADIUS, split_cut=False,
    ...
    while index < radar.nsweeps:
        ...
        except ValueError: # Note: this is a quick fix - 
                           # we may just want to exclude data points where 
                           # NTDA is not able to be run
            if verbose: print("Unable to use ntda on given sweep, processing without")
            if use_ntda: # retry, without the use of ntda
              ## TODO: in development
              turbulence[sweep_range[0]:sweep_range[1]], glat, glon = \
                  calc_turb_sweep(radar, index, radius=radius, split_cut=dsw,
                                  verbose=verbose, xran=xran, yran=yran,
                                  name_dz=name_dz, name_sw=name_sw,
                                  use_ntda=False, beamwidth=beamwidth,
                                  gate_spacing=gate_spacing,
                                  compute_gate_pos=compute_gate_pos)
        finally:
            index += 1
